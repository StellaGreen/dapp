...
function Dapp() {
  ...
  const handleClickSetStorage = async () => {
    ...
    try {
      setIsLoading(true)
      let tx = await SMS.createNFT({sentence: inputValue, title: "My NFT", editBy: "", linkUrl: "", sentenceHash: ethers.utils.id(inputValue), timeStamp: new Date().getTime()})
      await tx.wait()
      toast({
            ...
            {web3State.chainId === 4 ? (
              <>
                ...
                <Spacer/>
                  ...
                  <Center pos="relative" size="m" h="100vh" as="i" marginTop="7rem" color="grey" paddingStart="20rem" paddingEnd="20rem" paddingBottom="2rem" paddingTop="2rem">
                  <Text as="samp">
